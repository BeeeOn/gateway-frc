...
const Poco::Timespan JablotronDevice::REFRESH_TIME_SUPPORTED_BEACON(-1);
const Poco::Timespan JablotronDevice::REFRESH_TIME_UNSUPPORTED_BEACON(9 * Timespan::MINUTES);

static const DevicePrefix PREFIX = DevicePrefix::PREFIX_JABLOTRON;

SharedPtr<JablotronDevice> JablotronDevice::create(uint32_t serialNumber)
{
...
{
	return DeviceID(PREFIX, serialNumber);
}
...
int JablotronDevice::extractBatteryLevel(const std::string &battery)
{
