...
void TestingCenter::handle(Command::Ptr cmd, Answer::Ptr answer)
{
	if (cmd->is<ServerDeviceListCommand>()) {
		...
		deviceInfo += "Vendor: " + command->vendor() + "\n";
		deviceInfo += "ProductName: " + command->productName() + "\n";

		for (auto &dataTypes : command->dataTypes()) {
			deviceInfo += "Type: " + dataTypes.type().toString() + " ";
			deviceInfo += "\tAttributes: ";

			if (dataTypes.attributes().empty()) {
				deviceInfo += "none";
				continue;
			}

			for (auto &attr : dataTypes.attributes())
				deviceInfo += attr.toString() + ", ";
